...
// 
//  g++ -o RDataFrameFilterMemory.exe RDataFrameFilterMemory.cpp `root-config --cflags --glibs`
//  
...
//   dataframe_data.Filter([pu,s](int nPU, double signalTruth){return nPU == pu && signalTruth == s;}, {"nPU", "signalTruth"})
  float pu = 3;
  auto dataframe_data_post = dataframe_data.Define("a_v_b", "a_v[b]").Filter([pu](float a_v_here_b_here){return a_v_here_b_here >= pu; }, {"a_v_b"});
//   auto dataframe_data_post = dataframe_data.Filter([pu](float a_v_here[], int b_here){return a_v_here[b_here] >= pu; }, {"a_v", "b"});
  std::cout << "count 3  = " << *(dataframe_data_post.Count()) << std::endl;
